...
API_TOKEN_ABUSEIPDB = '6e055355e503033b59018c47e5b8536e88345d31377815ad6d17b9468866c6da3ce0433f8f586f41'
API_TOKEN_VT = 'ab232abf37ff843d9fef731011dc27f4f8d5cd30a18792af516fb1f2508eb288'
API_TOKEN_GREYNOISE = 'cpDHlt8response_json0ySYXhX06SLW7OPbxrmvd0QFz3IESNaxW61u2frSBz90EFsFVLmLn6So'
ELASTIC_USER='admin'
...
def abuseipdb_check(susp_ip):
    ...
    headers = {
        'Key':API_TOKEN_ABUSEIPDB,
        "Accept":"application/json"
    }
    try:
        response = requests.get('https://api.abuseipdb.com/api/v2/check', params=params, headers=headers, verify=False)
...
def virustotal_check(susp_ip):
    ...
    try:
        response = requests.get(f'https://www.virustotal.com/api/v3/ip_addresses/{susp_ip}', headers=headers, verify=False)
        response_json = json.loads(response.text)
        result['virustotal_malicious'] = response_json['data']['attributes']['last_analysis_stats']['malicious']
        return result
    except:
...
def hosting_check(susp_ip):
    ...
    params = {
        'ip':susp_ip, 'apikey':API_TOKEN_VT
    }
    try:
        response = requests.get('http://www.virustotal.com/vtapi/v2/ip-address/report', params=params, verify=False)
        response_json = json.loads(response.text)
...
def greynoise_check(susp_ip):
    ...
    headers = {
        'accept': 'application/json',
        'key': API_TOKEN_GREYNOISE
    }
    try:
        response = requests.get(f'https://api.greynoise.io/v3/community/{susp_ip}', headers=headers, verify=False)
        response_json = json.loads(response.text)
...
def elasticsearch_count(query):
    ...
    try:
        r = requests.post(f'https://{ELASTIC_IP}:9200/_count', data=query, headers=headers, auth=auth, verify=False)
        j = json.loads(r.text)
        return j['count']
    except:
        return 'Error'
...
def elasticsearch_search(query, size=50):
    ...
    auth = (ELASTIC_USER, ELASTIC_PASS)
    try:
        r = requests.post(f'https://{ELASTIC_IP}:9200/_search?size={size}', data=query, headers=headers, auth=auth, verify=False)
    ...
    query = '''{
        "query": {
            "bool": {
                "must": [{
                        "match": {
                            "data_win_eventdata_sourceIp": "''' + src_ip + '''"
                        }
                    },
                    {
                        "match": {
                            "data_win_eventdata_destinationIp": "''' + susp_ip + '''"
                        }
                    },
                    {
                        "range": {
    ...
    query = '''{
        "query": {
            "bool": {
                "must": [{
                        "match": {
                            "data_win_eventdata_sourceIp": "''' + susp_ip + '''"
                        }
                    },
                    {
                        "range": {
...
def elasticsearch_investigation(susp_ip, src_ip):
    ...
    result['count_30m_from_src_ip'] = elasticsearch_count_30m_from_src_ip(susp_ip, src_ip)
    result['count_24h_from_src_ip'] = elasticsearch_count_24h_from_src_ip(susp_ip, src_ip)
    result['count_24h_from_all_ip'] = elasticsearch_count_24h_from_all_ip(susp_ip, src_ip)
    result['count_hosts_to_susp_ip'], result['hosts_to_susp_ip'] = elasticsearch_hosts_to_susp_ip(susp_ip, src_ip)
    result['count_hosts_from_susp_ip'], result['hosts_from_susp_ip'] = elasticsearch_hosts_from_susp_ip(susp_ip, src_ip)
...
def investigation(susp_ip, src_ip):
    ...
    res_of_invest = {**hosting_check(susp_ip), **abuseipdb_check(susp_ip), **virustotal_check(susp_ip), **greynoise_check(susp_ip), **elasticsearch_investigation(susp_ip, src_ip)}
    message = ''
    if len(res_of_invest.values()) == 1 and res_of_invest.values()[0] == 'Connection error or Request limit':
        message = 'нет данных'
    elif res_of_invest['domains_number'] != 'Connection error or Request limit' and res_of_invest['domains_number'] > 50:
        message = 'FP (предположительно хостинг)'
    elif res_of_invest['is_whitelisted'] != 'Connection error or Request limit' and res_of_invest['is_whitelisted']:
        message = 'FP (в белом списке AbuseIPDB)'
    elif (res_of_invest['abuse_score'] != 'Connection error or Request limit' and res_of_invest['abuse_score'] < 10) \
          and (res_of_invest['virustotal_malicious'] != 'Connection error or Request limit' and res_of_invest['virustotal_malicious'] <= 1) \
          and (res_of_invest['greynoise_classification'] != 'Connection error or Request limit' and res_of_invest['greynoise_classification'] != 'malicious'):
...
def demo_print(susp_ip, src_ip):
    ...
    print(f'Количество запросов с {src_ip} к {susp_ip} за последние 30 минут: {result["count_30m_from_src_ip"]}')
    print(f'Количество запросов с {src_ip} к {susp_ip} за последние 24 часа: {result["count_24h_from_src_ip"]}')
    print(f'Количество запросов со всех хостов к {susp_ip} за последние 24 часа: {result["count_24h_from_all_ip"]}')
    print(f'Запросов к {susp_ip} за последние 24 часа были с {result["count_hosts_to_susp_ip"]} хостов: {result["hosts_to_susp_ip"]}')
    print(f'Запросов с {susp_ip} за последние 24 часа были к {result["count_hosts_from_susp_ip"]} хостам: {result["hosts_from_susp_ip"]}')
