...
categories = list(product(list(range(d)), list(range(d))))
series = [(Y[2*i], Y[2*i+1]) for i in range(n//2)]

table = [(categories[i], series.count(categories[i]), (n/2)*(1/d**2)) for i in range(len(categories))]

print('  r   |  Yi |npi')
for i in range(len(table)):
    print(str(table[i][0]).rjust(6), str(table[i][1]).rjust(4), str(table[i][2]).rjust(4))
